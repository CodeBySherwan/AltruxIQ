...
Label dicts, SI<->display conversion factors, and batch scaling helpers.
All internal computation is base SI; conversion happens only at display/input
boundaries via :func:`get_divisor` / :func:`to_si`.

This module supersedes the previously duplicated unit code in:
  - ``ui/Menus.py``    (METRIC_UNITS, IMPERIAL_UNITS, get_divisor, get_inverse_multiplier)
  - ``ui/cli.py``      (METRIC_LABELS, IMPERIAL_LABELS)
  - ``ui/inputs.py``   (CONVERSION_TO_SI)
